...
pokemons.forEach((pokemon, index) => {
    pokemon.addEventListener('click', () => {
        ...
        console.log(`monster ${index + 1}의 능력치는 ${stats}입니다.`);
    });
});

function compareMonsters(monster1Index, monster2Index) {
    if (monster1Index < 1 || monster1Index > totalMonsters || monster2Index < 1 || monster2Index > totalMonsters) {
    ...
    if (monster1Stats > monster2Stats) {
        console.log(`${monster1Index}이 이깁니다.`);
    } else if (monster2Stats > monster1Stats) {
        console.log(`${monster2Index}이 이깁니다.`);
    } else {
        console.log("무승부입니다.");
    }
}
